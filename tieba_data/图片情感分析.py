...
def analyze_emotion(image_path):
    try:
        ...
        if isinstance(result, list):
            result = result[0]  # If result is a list, take the first element
        return result.get('emotion')
    except Exception as e:
        print(f"Failed to analyze emotion for image {image_path}. Error: {e}")
        return None

# Analyze emotions for all images in the images folder and update JSON data
def analyze_emotions_in_folder(folder_path, data):
    ...
    for post in data:
        for image_path in post['images']:
            ...
            if image_file in image_files:
                emotion_result = analyze_emotion(os.path.join(folder_path, image_file))
                if emotion_result:
                    print(f"Image: {image_file} - Emotion Analysis: {emotion_result}")
                    post.setdefault('emotions', []).append({image_file: emotion_result})

# Main function to scrape posts, download images, and analyze emotions
def main():
    keyword = "福州大学"
    ...
    # Analyze emotions in the images folder and update JSON data
    analyze_emotions_in_folder(images_folder, data)

    # Write updated data to JSON file
    with open('images_emotions.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)
    print("Data has been written to posts.json")
